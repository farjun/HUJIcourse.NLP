...
class MSTParser:
    ...
    def get_full_graph_from_dict(self, sentence_dict):
        ...
        for fromIndex in range(total_indexes):
            ...
            for toIndex in range(total_indexes):
                ...
                arcs.append(
                    MSTAlgorithem.Arc(fromIndex, -1 * (self.getWordsWeight(word1['word'], word2['word']) +
                                      self.getTagsWeight(word1['tag'], word2['tag'])), toIndex))
        return arcs
    ...
    def getWordsWeight(self, w1, w2):
        ...
        if w2 not in self.word_weight[w1]:
            ...
            self.total_word_weight[w1][w2] = 0
